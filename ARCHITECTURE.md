...
- If Zed Settings file is not found, force user to input the Github token and Gist ID manually on the terminal, hiding the Github token by using rpassword crate.
  After that, create the Zed Settings file with the provided token and Gist ID. And then, sync the settings files from the Gist.
- ❌ Obsolete: no `zed-settings-sync load –-auth-token <GitHub auth token> –-gist <Gist ID>` options for the `load` command.
...
On any command that requires interaction with Github:
