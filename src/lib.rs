...
impl ZedSettingsSyncExtension {
    fn fallback(&mut self) -> zed::Result<String> {
        ...
        let binary_path: String = "zed-settings-sync-lsp".to_string();
    ...
    fn language_server_binary_path(
        ...
        if let Some(path) = worktree.which("zed-settings-sync-lsp") {
            return Ok(path);
        }
        ...
        let asset_name = format!(
            "zed-settings-sync-lsp-{arch}-{os}.{extension}",
            arch = match arch {
        ...
        let version_dir = format!("zed-settings-sync-lsp-{}", release.version);
        ...
        let binary_path: String = match platform {
            zed::Os::Windows => format!("{version_dir}/{asset_name}/zed-settings-sync-lsp.exe"),
            _ => format!("{version_dir}/{asset_name}/zed-settings-sync-lsp"),
        };
        ...
        if !fs::metadata(&binary_path).is_ok_and(|stat| stat.is_file()) {
            ...
            let _ = fs::remove_file("zed-settings-sync-lsp");
        }

        if !fs::metadata("zed-settings-sync-lsp").is_ok_and(|stat| stat.is_file())
            && let Err(e) = create_symlink(&binary_path, "zed-settings-sync-lsp")
        {
...
impl zed::Extension for ZedSettingsSyncExtension {
    fn new() -> Self {
        Self {
...
zed::register_extension!(ZedSettingsSyncExtension);
